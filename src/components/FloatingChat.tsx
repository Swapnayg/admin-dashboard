...
{(!isOpen || isCollapsed) && (
  <button
    onClick={openChat}
    className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 md:bottom-10 md:right-10 z-50 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-indigo-500 text-white shadow-lg hover:bg-indigo-600 transition-colors"
  >
    <MessageCircle className="h-5 w-5 sm:h-6 sm:w-6" />

    {totalUnread > 0 && (
      <span className="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] sm:text-xs text-white shadow">
      ...
      {isOpen && (
  <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[90%] sm:w-80 max-w-sm rounded-xl bg-white shadow-xl border border-gray-200 overflow-hidden">
    <div className="flex items-center justify-between px-4 py-2 bg-indigo-500 text-white">
      <span className="font-semibold">
        Chat ({username.charAt(0).toUpperCase() + username.slice(1)})
      </span>
    ...
    <div className="flex flex-col h-[70vh] sm:h-[25rem]">
      {!activeChat ? (
        <>
          <div className="text-sm text-gray-700 flex-1 overflow-y-auto space-y-3">
      ...
      {showGroupForm && (
  <div className="fixed bottom-24 right-4 sm:right-6 md:right-10 z-50 w-[90%] sm:w-[22rem] max-w-sm rounded-xl bg-white shadow-xl border border-gray-300 p-4">
    <div className="flex justify-between items-center mb-2">
