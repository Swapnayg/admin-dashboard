    ...
    <div className="space-y-4 w-full">
      ...
      <div className="space-y-2 overflow-x-auto">
        {Array.from({ length: rows }, (_, rowIndex) => (
          <div key={rowIndex} className="flex gap-1 overflow-x-auto">
          ...
          <div>
            ...
            <div className="space-y-2">
              {question.options.map((option, index) => (
                <div
                  key={index}
                  className="flex items-center space-x-3 p-2 rounded-md border border-slate-200"
                >
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <p className="text-slate-600 text-base sm:text-lg">Loading quiz results...</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="text-center max-w-sm w-full">
        ...
        <h3 className="text-lg sm:text-xl font-medium text-slate-900 mb-2">No quiz results found</h3>
        <p className="text-slate-600 text-sm sm:text-base mb-4">Unable to load quiz results data.</p>
        <Button onClick={onBackToStart} variant="outline" className="w-full sm:w-auto">
...
// After selectedAttempt is loaded:
const percentage = Math.round((selectedAttempt.obtainedMarks / selectedAttempt.totalMarks) * 100);
  
  return (
   <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
  {/* Sidebar - Question Navigator */}
  <div className="w-full md:w-80 bg-white border-b md:border-b-0 md:border-r border-slate-200 p-4 md:p-6">
    <ScrollArea className="h-full md:h-[calc(100vh-0px)]">
